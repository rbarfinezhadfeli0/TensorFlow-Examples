# Documentation: examples/6_MultiGPU/multigpu_cnn.py

## File Metadata

- **File Path**: `examples/6_MultiGPU/multigpu_cnn.py`
- **File Size**: 7873 bytes
- **File Type**: .py
...
This file is located at `examples/6_MultiGPU/multigpu_cnn.py` within the TensorFlow Examples repository.
...
Located in the `examples/` directory, specifically within `examples/6_MultiGPU/`, this file is part of the main examples collection (defaulting to latest TensorFlow).
...
This function handles average gradients functionality.

#### `assign_to_device(device, ps_device='/cpu:0')`

This function handles assign to device functionality.

#### `_assign(op)`

This function handles  assign functionality.
...
```bash
python examples/6_MultiGPU/multigpu_cnn.py
```
...
- [multigpu_basics.py](multigpu_basics.py_docs.md)
...
training, gradient, relu, neural network, assign_to_device, convolution, import, _assign, pooling, activation, accuracy, mnist, softmax, tensorflow.examples.tutorials.mnist, conv_net, __future__, division, if, prediction, time, loss, input_data, optimizer, average_gradients, testing, tensorflow, tensorflow
